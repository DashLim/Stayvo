-- Optional direct booking website URL (guest portal footer, alongside social icons)
alter table public.properties
  add column if not exists social_direct_booking_url text;
...
returns table (
  ...
  host_whatsapp_chat_number text,
  ...
  social_airbnb_url text,
  social_direct_booking_url text
)
language sql
security definer
...
as $$
  select
    ...
    p.social_facebook_url,
    p.social_instagram_url,
    p.social_x_url,
    p.social_tiktok_url,
    p.social_youtube_url,
    p.social_airbnb_url,
    p.social_direct_booking_url

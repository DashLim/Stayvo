-- Uploadable guest images (Supabase Storage), optional legacy Google Drive URL kept for old rows

alter table public.property_check_in_steps
add column if not exists guest_image_path text;
...
-- Storage: public read for guest portal; uploads scoped to {user_id}/{property_id}/...
insert into storage.buckets (id, name, public)
values ('guest-property-media', 'guest-property-media', true)
on conflict (id) do update set public = excluded.public;

drop policy if exists "Guest property media public read" on storage.objects;
create policy "Guest property media public read"
on storage.objects for select
using (bucket_id = 'guest-property-media');

drop policy if exists "Owners insert guest property media" on storage.objects;
create policy "Owners insert guest property media"
on storage.objects for insert to authenticated
with check (
  bucket_id = 'guest-property-media'
  and split_part(name, '/', 1) = auth.uid()::text
);

drop policy if exists "Owners update guest property media" on storage.objects;
create policy "Owners update guest property media"
...
drop policy if exists "Owners delete guest property media" on storage.objects;
create policy "Owners delete guest property media"
